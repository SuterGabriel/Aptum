...
| C4 | Solide Kenntnisse der relevanten Cloud- und DevOps-Praktiken (u. a. Kubernetes, Helm, ArgoCD, Terraform) | in Arbeit | Drei Dienste als mehrstufige Container ohne Root (`services/scheduling/Dockerfile`, `services/ai-assist/Dockerfile`, `frontend/Dockerfile`), `compose.yml` mit Healthchecks und Startbedingungen; Helm-Chart in `deploy/helm/aptum/` mit getrennten Liveness- und Readiness-Probes, das die CI (Job `cluster`) bei jedem Push in einen `kind`-Cluster installiert und mit `helm test` prüft; Terraform und ArgoCD folgen |
